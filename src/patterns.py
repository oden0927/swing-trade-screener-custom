...
def evaluate(df: pd.DataFrame, env: EnvironmentResult) -> PatternResult:
    ...
    # ============================================================
    # 【撤回】B6/N5 ボリンジャーバンド系のスコア反映【2026-05-24 撤回】
    # 事前検証 (test_bollinger.py): B6=勝率56.4%、N5=勝率49.9% で有望と判定
    # しかし実際のスクリーナー候補集合での3年実証で結果が逆転:
    #   B6 スクイーズ→エクスパンション: 516件、勝率49.4%、-0.10% (逆効果)
    #   N5 +2σ超 過熱:                1152件、勝率53.0%、+0.66% (減点する価値なし)
    # 理由: スクリーナーは既に「動き出した上昇トレンド」銘柄に絞り込んだ集合のため、
    #       事前検証の全サンプル前提とBB シグナルの意味が変化する。
    # 対応: スコア計算からは撤回。BB の計算と表示（チャート/UI）は人間の参考用に残す。
    # ============================================================
